...
const CreateBook = () => {
    ...
    useEffect(() => {
        const pending = localStorage.getItem('pending_book');
        if (pending) {
            try {
                const parsed = JSON.parse(pending);
                setValues(parsed);
                toast.success('Draft data restored');
            } catch (e) {
                console.error('Error parsing pending book from localStorage:', e);
            }
        }
    }, []);
    ...
    const handleSubmit = (e) => {
        ...
        if (!isLoggedIn) {
            localStorage.setItem('pending_book', JSON.stringify(values));
            toast.error('Please login first to submit');
        ...
        axios.post(`${import.meta.env.VITE_API_URL}/create`, payload)
            .then(res => {
            ...
            .catch(err => {
                console.error(err);
                setApiError(err.response?.data?.error || 'Failed to create book. Please verify your authentication.');
            })
